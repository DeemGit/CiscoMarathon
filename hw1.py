...
def get_previous_backup_file_path(hostname, curent_backup_file_path):
    ...
    current_backup_filename = curent_backup_file_path.split(os.path.sep)[-1]

    # creatting an empty dictionary to keep backup file names
    last_backup_file = sorted(filter(lambda f: f != current_backup_filename, os.listdir(os.path.dirname(curent_backup_file_path))), key=lambda f: f.split("-"), reverse=True)
    return os.path.dirname(curent_backup_file_path) + os.path.sep + last_backup_file[0] if last_backup_file else False
    '''
...
def main(*args):
    ...
    '''
    for device in device_list:
        process_target(device, timestamp)
